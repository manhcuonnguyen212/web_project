// Get monthly user stats for dashboard
export const getUserStats = async (req, res) => {
  try {
    ...
    const userQuery = { role: { $nin: ["admin", "supervisor admin"] } };
    const thisMonthCount = await UserModel.countDocuments({
      ...userQuery,
...
export const toggleStatus = async (req, res) => {
  ...
  try {
    ...
    // Chỉ trả về thông báo, không xoá cookie của admin
    res.status(200).json({
      success: true,
      message: wasBlocked ? "Đã mở khoá tài khoản thành công." : "Đã khoá tài khoản thành công.",
      data: rest,
    });
  } catch (error) {
    res.status(500).json({
...
export const getSavedPosts = async (req, res) => {
  try {
    ...
    const savedPosts = await NewsModel.find({
      savedBy: userId,
      status: "published",
    })
      .select("-content")
...
export const getLikedPosts = async (req, res) => {
  try {
    ...
    const likedPosts = await NewsModel.find({
      likedBy: userId,
      status: "published",
    })
      .select("-content")
...
export const getCommentedPosts = async (req, res) => {
  try {
    ...
    // Tìm tất cả comments của user theo username
    const userComments = await CommentModel.find({ author: user.username })
      .distinct("newsId");

    // Tìm các bài viết từ newsIds
    const commentedPosts = await NewsModel.find({
      _id: { $in: userComments },
      status: "published",
    })
      .select("-content")
